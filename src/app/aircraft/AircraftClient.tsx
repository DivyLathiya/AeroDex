...
export default function AircraftClient({ initialFamilies }: { initialFamilies: any[] }) {
  ...
  const families = useMemo(() => {
    return initialFamilies.filter(f => {
      ...
      if (mfr && f.manufacturer !== mfr) return false;
      if (type && f.type !== type) return false;
      return true;
    });
  }, [initialFamilies, q, mfr, type]);
        ...
        <div className="space-y-6">
          <div className="bg-card border border-white/5 p-6 rounded-2xl space-y-6 sticky top-24">
            ...
            <div>
              ...
              <select value={type} onChange={(e) => setType(e.target.value)} className="w-full bg-background border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary">
            ...
            {(q || mfr || type) && (
              <button onClick={() => { setQ(''); setMfr(''); setType(''); }} className="w-full block text-center text-sm text-muted-foreground hover:text-white mt-2">
                Clear Filters
              </button>
            )}
          </div>
